...
#data = HDF5DataLayer(name="train-data", source="data/etc/text.done.string.data");
#conv = ConvolutionLayer(name="conv", n_filter=20, kernel=(5,5), bottoms=[:data], tops=[:conv]);
using Mocha
srand(12345678)

data_layer  = AsyncHDF5DataLayer(name="train-data", source="tmp/test.txt", batch_size=64, shuffle=true)
